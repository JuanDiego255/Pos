...
use App\Events\UpdateKitchenEvent;
use App\Events\UpdateKitchenFinishEvent;
...
use App\Models\DetailKitchenOrder;
...
class OrderController extends Controller
{
    ...
    public function get()
    {
        $orders = Order::select(
            'orders.*',
            DB::raw('IFNULL(users.user, "Sin Mesero") as mesero'), // Reemplaza usuario NULL
            DB::raw('IFNULL(tables.descripcion, "N/A") as mesa')   // Reemplaza mesa NULL
        )
            ->leftJoin('users', 'orders.idusuario', '=', 'users.id') // LEFT JOIN para incluir órdenes sin usuario
            ->leftJoin('tables', 'orders.idmesa', '=', 'tables.id') // LEFT JOIN para incluir órdenes sin mesa
            ...
            ->addColumn('acciones', function ($orders) {
                ...
                if ($estado == 0) {
                    $btn .= '<a class="dropdown-item" href="' . route('admin.register_order', $idmesa) . '" data-id="' . $id . '"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye mr-50 menu-icon"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>
                                        ...
                                        <span>Generar Comprobante (Cancelar)</span></a>';
                } elseif ($estado == 1) {
                ...
                $html .= '<div class="col-md-3 mb-3">
                            <div class="card mb-6">
                                <div class="card-title p-3 header-elements alert alert-info">
                                    <h5 class="m-0 me-2">' . $descripcionMesa . '</h5>
    ...
    public function gen_ticket_sn($id, $name)
    {
        ...
        $idpais                     = (Business::where('id', 1)->first()->idpais == null) ? 4 : Business::where('id', 1)->first()->idpais;
        $data["moneda_pais"]        = CountryU::where('id', $idpais)->first()->signo;
        $data["descripcion_moneda_pais"]        = CountryU::where('id', $idpais)->first()->moneda;

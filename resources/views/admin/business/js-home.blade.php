    ...
    function load_country() {
        $.ajax({
            ...
            success     : function(r){
                ...
                let idpais      = r.business.idpais,
                    countries   = r.countries,
                    html        = '<option></option>';

                $.each(countries, function(index, country) {
                    html += `<option value="${country.id}">${country.descripcion} - ${country.signo}</option>`;
                });    
                if(idpais == null) {
                    $('select[name="idpais_u"]').html(html).select2({
                        placeholder     : "[SELECCIONE]",
                    });
                } else {
                    $('select[name="idpais_u"]').html(html).select2();
                    $('select[name="idpais_u"]').val(idpais).change();
                }
            },
            dataType    : "json"
        });
